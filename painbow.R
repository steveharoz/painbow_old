# Testing XKCD's supposedly "bad" colormap https://xkcd.com/2537/
# By Steve Haroz
# License: CC-BY

library(tidyverse)
library(patchwork) # for combining charts
library(viridis) # for comparison
...
data = expand.grid(
  ...
  mutate(znoise = z + dnorm(sqrt((x-0.75*COUNT)^2 + (y-0.33*COUNT)^2)/COUNT*20))   #  sin(x) + cos(y))

ggplot(data) +
  aes(x=x, y=y, fill=znoise) + 
  geom_tile(width=1, height=1, color=NA) +
  labs(title = "ggplot default", fill=NULL) + 
  theme_void(20) + theme(legend.text = element_blank()) +
ggplot(data) +
  aes(x=x, y=y, fill=znoise) + 
  geom_tile(width=1, height=1, color=NA) +
  scale_fill_viridis() +
...
ggplot(data) +
  ...
  theme_void(20) + theme(legend.text = element_blank()) +
  patchwork::plot_annotation(title = "Three colormaps. Same data. Can you spot the weird region?", theme = theme(text = element_text(size=30)))
...
ggplot(data) +
  ...
  geom_tile(width=1, height=COUNT, color=NA) +
  scale_fill_painbow() +
  labs(title = "xkcd's colormap: y=x") + 
  theme_void(20) + theme(legend.text = element_blank()) +
...
ggplot(data) +
  ...
  geom_tile(width=1, height=COUNT, color=NA) +
  labs(title = "ggplot default: y=x + harmonic") + 
  theme_void(20) + theme(legend.text = element_blank()) +
...
ggplot(data) +
  ...
  geom_tile(width=1, height=COUNT, color=NA) +
  scale_fill_viridis() +
  labs(title = "viridis: y=x + harmonic") + 
  theme_void(20) + theme(legend.text = element_blank()) +
...
ggplot(data) +
  ...
  geom_tile(width=1, height=COUNT, color=NA) +
  scale_fill_painbow() +
  labs(title = "xkcd's colormap: y=x + harmonic") +
  theme_void(20) + theme(legend.text = element_blank()) +
